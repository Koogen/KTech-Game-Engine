...
{
	int mousex,mousey,x,y,xSize,ySize,texty,textx,bColor,tColor;
	String text;
	boolean isPressed = false;
	
	public Button(int x, int y, int xSize, int ySize, String text,int textx, int texty, int bColor, int tColor){
		this.x = x;
		this.y = y;
		this.xSize = xSize;
		this.ySize = ySize;
	...
	public void update(KTech kt, float time) {
		...
		if(((mousex >= x)&&(mousex <= x + xSize))&&((mousey >= y)&&(mousey <= y + ySize))&& kt.getInput().isMBPressed(1)){
			isPressed = true;
		}
		else{
			isPressed = false;
